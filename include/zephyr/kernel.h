...
#ifdef CONFIG_COMMON_LIBC_MALLOC_TLS
/**
 * @brief Assign a local malloc heap to a thread
 *
 ...
 */
static inline int k_thread_local_heap_assign(struct k_thread *thread, struct sys_heap_local *heap)
{
	thread->local_heap = heap;
	return 0;
}

/**
 * @brief Get the local heap for the thread
 *
 ...
 */
__syscall struct sys_heap_local *k_thread_local_heap_get(struct k_thread *thread);
#endif /* CONFIG_COMMON_LIBC_MALLOC_TLS */
